...
exports.run = async (client, message, args) => {
  if (
    ![ayarlar.registeryetki].some(role =>
  ...
  if (member.id === message.author.id)
    return message.channel.send("Kendini kayıt edemezsin Malmısın ");
  if (member.id === client.user.id)
    return message.channel.send("Botu kayıt edemezsin.");
  if (member.id === message.guild.OwnerID)
    return message.channel.send(
      "Sunucu sahibini kayıt edemezsin O Zaten Sunucu Sahibi"
    );
  if (member.roles.highest.position >= message.member.roles.highest.position)
    return message.channel.send(`Bu kullanıcı sizden üst/aynı pozsiyondadır.`);
  ...
  member.roles.add(ayarlar.kadınrol);
  
  member.roles.remove(ayarlar.kayıtsız);

  const a = await datab.fetch(`Eskiİsimler.${message.guild.id}.${member.id}`);
  if (a == null)
    await datab.set(`Eskiİsimler.${message.guild.id}.${member.id}`, []);
  await datab.push(
    `Eskiİsimler.${message.guild.id}.${member.id}`,
    `${name} ${yas}`
  );

  let fdh = datab.fetch(`Eskiİsimler.${message.guild.id}.${member.id}`);
  if (!fdh | (fdh === []) || fdh.length === 0 || fdh.length < 1)
    return message.channel
      .send("Eski İsimleri Yok!")
  ...
  **Kayıta Verilen Rol** : <@&${ayarlar.kadınrol}>
  **Kayıta Alınan Rol** : <@&${ayarlar.kayıtsız}>
  **Yeni Kullanıcı Adı** :  \`${tag} ${name} | ${yas}\`
 
**Bu Kullanıcının Sunucudaki Eski İsimleri** [${sayı++ ||
        "0"}]     \n\n${data.join("\n")}  `
    )
    .setColor("0x348f36")
...
exports.help = {
  name: "kadın",
  description: "Belirttiğiniz kişiyi sunucuda kadın rolü verir.",
  usage: "KADINNN"
};
