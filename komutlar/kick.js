...
var prefix = ayarlar.prefix;             
  ...
  if (!message.member.permissions.has("KICK_MEMBERS")) return message.channel.send(new Discord.MessageEmbed() .setTitle("Hata!") .setDescription("**Bu komutu kullanamazsınız!**"));
  ...
  if (message.mentions.users.size < 1) return message.channel.send(new Discord.MessageEmbed() .setTitle("Hata!") .setDescription("**Birini etiketlemeyi unuttunuz.**"));
...
if (user.position > message.member.roles.highest.position) return message.channel.send(`Bu kullanıcının senin rollerinden/rolünden daha yüksek rolleri/rolü var.`);
 ...
 if (!message.guild.member(user).bannable) return message.channel.send(new Discord.MessageEmbed() .setTitle("Hata!") .setDescription("**Etiketlediğiniz kişiyi atamıyorum.**"));
